...
public class JSON {
	...
	public static JValue parse(String data) {
		return readValue(new Buffer(data));
	}
	...
	public static JList<JList<JString>> make2DStringList(ArrayList<? extends Object[]> values) {
		@SuppressWarnings("unchecked") // oh my god why java
		JList<JString>[] items = new JList[values.size()];
		for (int i = 0; i < values.size(); i++) {
			items[i] = makeStringList(values.get(i));
		}
		return new JList<JList<JString>>(items);
	}
}